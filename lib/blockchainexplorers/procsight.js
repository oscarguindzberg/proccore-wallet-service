...
var _parseErr = function(err, res) {
  if (err) {
    log.warn('Procsight error: ', err);
    return "Procsight Error";
  }
  log.warn("Procsight " + res.request.href + " Returned Status: " + res.statusCode);
  return "Error querying the blockchain";
};

Procsight.prototype._doRequest = function(args, cb) {
  var opts = {
    hosts: this.hosts,
    headers: {
...
Procsight.prototype.getConnectionInfo = function() {
  return 'Procsight (' + this.coin + '/' + this.network + ') @ ' + this.hosts;
};

/**
 * Retrieve a list of unspent outputs associated with an address or set of addresses
 */
Procsight.prototype.getUtxos = function(addresses, cb) {
...
/**
 * Broadcast a transaction to the procurrency network
 */
Procsight.prototype.broadcast = function(rawTx, cb) {
  var args = {
    method: 'POST',
...
Procsight.prototype.getTransaction = function(txid, cb) {
  var args = {
    method: 'GET',
...
Procsight.prototype.getTransactions = function(addresses, from, to, cb) {
  var qs = [];
  var total;
  ...
  this._doRequest(args, function(err, res, txs) {
    ...
    // NOTE: Whenever Procsight breaks communication with procurrencyd, it returns invalid data but no error code.
...
Procsight.prototype.getAddressActivity = function(address, cb) {
  var self = this;
...
Procsight.prototype.estimateFee = function(nbBlocks, cb) {
...
Procsight.prototype.getBlockchainHeight = function(cb) {
...
Procsight.prototype.getTxidsInBlock = function(blockHash, cb) {
  var self = this;
...
Procsight.prototype.initSocket = function() {
