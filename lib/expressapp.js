  ...
  router.get('/v1/wallets/', function(req, res) {
    ...
    getServerWithAuth(req, res, function(server) {
      server.getStatus({
        ...
        if (err) return returnError(err, res, req);
        cleanPendingTxs(status.pendingTxps);
        res.json(status);
      });
    });
  });
    ...
    getServerWithAuth(req, res, function(server) {
      ...
      server.getStatus(opts, function(err, status) {
        ...
        cleanPendingTxs(status.pendingTxps);
          ...
          status.balance.totalAmount = parseInt(status.balance.totalAmount.toString());
          status.balance.lockedAmount = parseInt(status.balance.lockedAmount.toString());
          status.balance.totalConfirmedAmount = parseInt(status.balance.totalConfirmedAmount.toString());
          status.balance.lockedConfirmedAmount = parseInt(status.balance.lockedConfirmedAmount.toString());
          status.balance.availableAmount = parseInt(status.balance.availableAmount.toString());
          status.balance.availableConfirmedAmount = parseInt(status.balance.availableConfirmedAmount.toString());
          status.balance.byAddress.forEach((address)=>{address.amount = parseInt(address.amount.toString())});
        res.json(status);
      });
    });
  });
  ...
  router.post('/v2/txproposals/', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      server.createTx(req.body, function(err, txp) {
        ...
        var convert_to_number = function(x){
            if (typeof(x) == 'string')
                return parseInt(x);
        ...
        //Fix up numbers before returning them to the client:
        for (var i = 0; i < txp.outputs.length; i++)
            txp.outputs[i].amount = convert_to_number(txp.outputs[i].amount);
        txp.feePerKb = convert_to_number(txp.feePerKb);
        txp.amount = convert_to_number(txp.amount);
        for (var i = 0; i < txp.inputs.length; i++)
            txp.inputs[i].satoshis = convert_to_number(txp.inputs[i].satoshis);
        txp.fee = convert_to_number(txp.fee);
  ...
  router.get('/v1/utxos/', function(req, res) {
    ...
    getServerWithAuth(req, res, function(server) {
      server.getUtxos(opts, function(err, utxos) {
        if (err) return returnError(err, res, req);
        utxos.forEach(function(e,i){
          e.satoshis = parseInt(e.satoshis.toFixed())
        });
        res.json(utxos);
      });
    });
  ...
  router.post('/v1/txproposals/:id/publish/', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      ...
      server.publishTx(req.body, function(err, txp) {
        ...
        txp.amount = parseInt(txp.amount);
        txp.fee = parseInt(txp.fee);
        txp.outputs.forEach((out)=>{
          out.amount = parseInt(out.amount);
        });
        txp.inputs.forEach((inp)=>{
          inp.satoshis = parseInt(inp.satoshis);
        });
        res.json(txp);
        res.end();
      });
    ...
    getServerWithAuth(req, res, {
      ...
      server.getNotifications(opts, function(err, notifications) {
        ...
        notifications.forEach((noti)=>{
          if((noti.type == "NewOutgoingTx" || noti.type == "NewIncomingTx") && BigNumber.isBigNumber(noti.data.amount)) noti.data.amount = parseInt(noti.data.amount.toFixed());
        });
...
function cleanPendingTxs(txs){
  txs.forEach(function(tx){
    tx.fee = parseInt(tx.fee.toFixed());
    tx.amount = parseInt(tx.amount.toFixed());
    tx.outputs.forEach(function(out){
      out.amount = parseInt(out.amount.toFixed());
    });
    tx.inputs.forEach(function(inp){
      inp.satoshis = parseInt(inp.satoshis.toFixed());
    });
  });
}
