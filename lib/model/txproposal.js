...
TxProposal.create = function(opts) {
  ...
  x.outputs = _.map(opts.outputs, function(output) {
    var ret = _.pick(output, ['amount', 'toAddress', 'message', 'script']);
    ret.amount = new BigNumber(ret.amount);
    return ret;
  });
...
TxProposal.prototype.toObject = function() {
  ...
  x.inputs.forEach(( el, i) =>{
    el.satoshis = this.inputs[i].satoshis;
  });
  x.outputs.forEach((el, i) => {
    el.amount = this.outputs[i].amount;
  });
...
TxProposal.prototype.setInputs = function(inputs) {
  //log.info('TxProposal.prototype.setInputs: ' + new Error().stack);
  if (inputs){
      //log.info('TxProposal.prototype.setInputs: ' + JSON.stringify(inputs));
      for (var i = 0; i < inputs.length; i++){
          log.info('TxProposal.prototype.setInputs: ' + (inputs[i].satoshis instanceof BigNumber) + ', ' + typeof(inputs[i].satoshis));
      }
  }
  this.inputs = inputs || [];
...
TxProposal.prototype._buildTx = function() {
  ...
  switch (self.addressType) {
    ...
    case Constants.SCRIPT_TYPES.P2PKH:
      self.inputs.get_log = function(){ return log; };
      t.from(self.inputs);
      break;
  }
  ...
  _.each(self.outputs, function(o) {
    ...
    log.info('_buildTx, o.amount: ' + JSON.stringify(o.amount));
    log.info('_buildTx, o.amount: ' + (o.amount instanceof BigNumber));
    if (o.script) {
      var output = new Bitcore[self.coin].Transaction.Output({
        script: o.script,
        satoshis: o.amount
      });
      log.info('output._satoshis = ' + (output._satoshis instanceof BigNumber));
      t.addOutput(output);
    } else {
      t.to(o.toAddress, o.amount);
      for (var i = 0; i < t.outputs.length; i++){
          const util = require('util')
          log.info('t.outputs[' + i + ']._satoshis = ' + (t.outputs[i]._satoshis instanceof BigNumber) + util.inspect(t.outputs[i]._satoshis, true, null, true));
      }
    }
  });
  ...
  var totalInputs = Utils.sum(t.inputs, 'output.satoshis');
  var totalOutputs = Utils.sum(t.outputs, 'satoshis');

  $.checkState(totalInputs.isGreaterThan(0));
  $.checkState(totalOutputs.isGreaterThan(0));
  $.checkState(totalInputs.isGreaterThanOrEqualTo(totalOutputs));
  $.checkState(totalInputs.minus(totalOutputs).isLessThanOrEqualTo(Defaults.MAX_TX_FEE));
...
TxProposal.prototype.getBitcoreTx = function() {
  var self = this;
  if(!this.inputs[0].satoshis.toFixed() || !this.outputs[0].amount.toFixed()) throw new Error()
...
TxProposal.prototype.getEstimatedFee = function() {
  return Utils.estimateFee(this.getEstimatedSize(), this.feePerKb);
};
...
TxProposal.prototype.getTotalAmount = function() {
  return Utils.sum(this.outputs, 'amount');
};
