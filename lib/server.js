...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb) {
  ...
  log.info('_selectTxInputs 5');

  function select(utxos, cb) {
    log.info('_selectTxInputs 20-2');
    var totalValueInUtxos = Utils.sum(utxos, 'satoshis');
    log.info('_selectTxInputs 20-1');
    //log.info('utxos: ' + JSON.stringify(utxos));
    //log.info('utxos.length: ' + JSON.stringify(utxos.length));
    var pf = procFee(utxos.length);
    log.info('pf: ' + JSON.stringify(pf));
    var netValueInUtxos = totalValueInUtxos.minus(pf);
    
    log.info('_selectTxInputs 20');
    ...
    _.each(smallInputs, function(input, i) {
      ...
      log.info('_selectTxInputs 31');

      if (txpSize.div('1000') > Defaults.MAX_TX_SIZE_IN_KB) {
    ...
    log.info('total: ' + typeof(total));
    log.info('total: ' + JSON.stringify(getMethods(total)));
    if ( total.minus(fee) < txpAmount) {
  ...
  self._getUtxosForCurrentWallet({}, function(err, utxos) {
    ...
    log.info('_selectTxInputs 9');

    log.info('Considering ' + utxos.length + ' utxos (' + Utils.formatUtxos(utxos) + ')');

    var groups = [6, 1];
    if (!txp.excludeUnconfirmedUtxos) groups.push(0);

    log.info('_selectTxInputs 10');
...
WalletService.prototype._getFeePerKb = function(wallet, opts, cb) {
  ...
  self.getFeeLevels({
    ...
    log.info(JSON.stringify(level));
    if (!level) {
      var msg = 'Could not compute fee for "' + opts.feeLevel + '" level';
      log.error(msg);
      return cb(new ClientError(msg));
    }
    log.info('level.feePerKb = ' + JSON.stringify(level.feePerKb));
    ...
    self.getWallet({}, function(err, wallet) {
      ...
      checkTxpAlreadyExists(opts.txProposalId, function(err, txp) {
        ...
        async.series([
          ...
          log.info('step 5: ' + JSON.stringify(err));
          if (err) return cb(err);
          log.info('step 6: ' /*+ JSON.stringify(txp)*/);
          return cb(null, txp);
        });
  ...
  function getNormalizedTxs(wallet, addresses, from, to, cb) {
    ...
    async.series([
      ...
      function(next) {
        ...
        log.info(_.uniq([].concat(addressStrs)).join(','))
        bc.getTransactions(addressStrs, from, to, function(err, rawTxs, total) {
          if (err) return next(err);
        log.info(util.inspect(rawTxs, true, null, true))

          txs = self._normalizeTxHistory(rawTxs);
        log.info(util.inspect(txs, true, null, true))
          totalItems = total;
          return next();
        });
      },
