...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb) {
  ...
  function procFee( n ) {
    return  (1 + Math.floor((baseTxpSize + (n * sizePerInput)) / 1000.)) * txp.feePerKb;
  }
  ...
  function select(utxos, cb) {
    ...
    var netValueInUtxos = totalValueInUtxos - procFee(utxos.length);
    ...
    var bigInputThreshold = txpAmount * Defaults.UTXO_SELECTION_MAX_SINGLE_UTXO_FACTOR + procFee(1);
    ...
    _.each(smallInputs, function(input, i) {
      ...
      if (!_.isEmpty(bigInputs)) {
        ...
        if (feeVsAmountRatio > Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR) {
          var feeVsSingleInputFeeRatio = fee / procFee(1);// (baseTxpFee + feePerInput);
