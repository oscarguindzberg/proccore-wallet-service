  ...
  function withSignature(cb) {
    ...
    server.storage.fetchCopayerLookup(opts.copayerId, function(err, copayer) {
      ...
      if (!copayer.isSupportStaff) {
        var isValid = !!server._getSigningKey(opts.message, opts.signature, copayer.requestPubKeys, opts.coin);
        if (!isValid)
 ...
 * @param pubKeys
 */
WalletService.prototype._verifySignature = function(text, signature, pubkey,coin) {
 ...
 * @param xPubKey
 */
WalletService.prototype._verifyRequestPubKey = function(requestPubKey, signature, xPubKey,opts) {
  opts.coin = opts.coin || Defaults.COIN;
  var pub = (new Bitcore_[opts.coin].HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
 ...
 * @param pubKeys
 */
WalletService.prototype._getSigningKey = function(text, signature, pubKeys, coin) {
  var self = this;
  return _.find(pubKeys, function(item) {
    return self._verifySignature(text, signature, item.key, coin);
  });
};
...
WalletService.prototype._notifyTxProposalAction = function(type, txp, extraArgs, cb) {
  ...
  var data = _.assign({
    ...
    amount: txp.getTotalAmount().toFixed(),
    message: txp.message,
  }, extraArgs);
...
WalletService.prototype.addAccess = function(opts, cb) {
  ...
  self.storage.fetchCopayerLookup(opts.copayerId, function(err, copayer) {
    ...
    self.storage.fetchWallet(copayer.walletId, function(err, wallet) {
      ...
      if (!self._verifyRequestPubKey(opts.requestPubKey, opts.signature, xPubKey,opts)) {
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  try {
    Bitcore_[opts.coin].HDPublicKey(opts.xPubKey);
  } catch (ex) {
  ...
  self._runLocked(cb, function(cb) {
    self.storage.fetchWallet(opts.walletId, function(err, wallet) {
      ...
      if (!self._verifySignature(hash, opts.copayerSignature, wallet.pubKey, opts.coin)) {
...
WalletService.prototype.verifyMessageSignature = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    var isValid = !!self._getSigningKey(opts.message, opts.signature, copayer.requestPubKeys,opts.coin);
...
WalletService.prototype._getUtxos = function(coin, addresses, cb) {
  ...
  bc.getUtxos(addresses, function(err, utxos) {
    ...
    var utxos = _.map(utxos, function(utxo) {
      ...
      u.locked = false;
      u.satoshis = u.satoshis ? new BigNumber(u.satoshis) : new BigNumber(u.amount).times('100000000');
      delete u.amount;
      return u;
    });
...
WalletService.prototype._totalizeUtxos = function(utxos) {
  var balance = {
    totalAmount: Utils.sum(utxos, 'satoshis'),
    lockedAmount: Utils.sum(_.filter(utxos, 'locked'), 'satoshis'),
    totalConfirmedAmount: Utils.sum(_.filter(utxos, 'confirmations'), 'satoshis'),
    lockedConfirmedAmount: Utils.sum(_.filter(_.filter(utxos, 'locked'), 'confirmations'), 'satoshis'),
  };
  balance.availableAmount = balance.totalAmount.minus(balance.lockedAmount);
  balance.availableConfirmedAmount = balance.totalConfirmedAmount.minus(balance.lockedConfirmedAmount);
...
WalletService.prototype.getSendMaxInfo = function(opts, cb) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self._getUtxosForCurrentWallet({}, function(err, utxos) {
      ...
      inputs = _.sortBy(inputs, function(x) {
        return new BigNumber(x.satoshis.toFixed()).negated();
      });
      ...
      self._getFeePerKb(wallet, opts, function(err, feePerKb) {
        ...
        var partitionedByAmount = _.partition(inputs, function(x) {
          return x.satoshis > feePerInput;
        });

        info.utxosBelowFee = partitionedByAmount[1].length;
        info.amountBelowFee = Utils.sum(partitionedByAmount[1], 'satoshis');
        ...
        _.each(inputs, function(input, i) {
          ...
          if (sizeInKb > Defaults.MAX_TX_SIZE_IN_KB) {
            ...
            info.amountAboveMaxSize = Utils.sum(_.slice(inputs, i), 'satoshis');
            return false;
          }
        ...
        var amount = Utils.sum(txp.inputs, 'satoshis') - fee;
...
WalletService.prototype._sampleFeeLevels = function(coin, network, points, cb) {
  ...
  bc.estimateFee(points, function(err, result) {
    ...
    var levels = _.zipObject(_.map(points, function(p) {
      var feePerKb = new BigNumber(result[p]);
      return [p, feePerKb.times('100000000')];
    }));
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    ...
    log.error('Error building Bitcore transaction', ex + ' stack ' + ex.stack);
    return ex;
  }
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb) {
  ...
  var baseTxpSize = txp.getEstimatedSize();
  var baseTxpFee = baseTxpSize * txp.feePerKb / 1000.;//  (1 + Math.floor(baseTxpSize / 1000)) * txp.feePerKb ;
  var sizePerInput = txp.getEstimatedSizeForSingleInput();
  var feePerInput = sizePerInput * txp.feePerKb / 1000.;

  function procFee( n ) {
    return Utils.estimateFee(baseTxpSize + (n * sizePerInput), txp.feePerKb);
  }
  ...
  function select(utxos, cb) {
    var totalValueInUtxos = Utils.sum(utxos, 'satoshis');
    var pf = procFee(utxos.length);
    var netValueInUtxos = totalValueInUtxos.minus(pf);
    
    if (totalValueInUtxos.isLessThan(txpAmount)) {
      ...
      console.log('totalValueInUtxos ' + totalValueInUtxos + " - " + typeof(totalValueInUtxos));
      return cb(Errors.INSUFFICIENT_FUNDS);
    }
    
    if (netValueInUtxos.isLessThan(txpAmount)) {
    ...
    var bigInputThreshold = procFee(1).plus(txpAmount.times(Defaults.UTXO_SELECTION_MAX_SINGLE_UTXO_FACTOR));
    log.debug('Big input threshold ' + Utils.formatAmountInBtc(bigInputThreshold));

    var partitions = _.partition(utxos, function(utxo) {
      return utxo.satoshis.isGreaterThan(bigInputThreshold);
    });
    ...
    var netTotal = new BigNumber(baseTxpFee).negated();
    var selected = [];
    var fee = procFee(selected.length);
    var error;

    _.each(smallInputs, function(input, i) {
      log.debug('Input #' + i + ': ' + Utils.formatUtxos(input));

      var input_satoshis = new BigNumber(input.satoshis.toFixed());

      var netInputAmount = input_satoshis.minus(feePerInput);

      log.debug('The input contributes ' + Utils.formatAmountInBtc(netInputAmount));

      selected.push(input);

      total = input_satoshis.plus(total);
      netTotal = netTotal.plus(netInputAmount);

      var txpSize = new BigNumber(baseTxpSize + selected.length * sizePerInput);
      fee = procFee(selected.length);// Math.round(baseTxpFee + selected.length * feePerInput);

      log.debug('Tx size: ' + Utils.formatSize(txpSize) + ', Tx fee: ' + Utils.formatAmountInBtc(fee));

      var feeVsAmountRatio = fee.div(txpAmount);
      var amountVsUtxoRatio = netInputAmount.div(txpAmount);
      ...
      if (txpSize.div('1000').isGreaterThan(Defaults.MAX_TX_SIZE_IN_KB)) {
      ...
      if (!_.isEmpty(bigInputs)) {
        if (amountVsUtxoRatio.isLessThan(Defaults.UTXO_SELECTION_MIN_TX_AMOUNT_VS_UTXO_FACTOR)) {
          log.debug('Breaking because utxo is too small compared to tx amount');
          return false;
        }

        if (feeVsAmountRatio.isGreaterThan(Defaults.UTXO_SELECTION_MAX_FEE_VS_TX_AMOUNT_FACTOR)) {
          var feeVsSingleInputFeeRatio = fee.div(procFee(1));// (baseTxpFee + feePerInput);
          log.debug('Fee/Single-input fee: ' + Utils.formatRatio(feeVsSingleInputFeeRatio) + ' (max: ' + Utils.formatRatio(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR) + ')' + ' loses wrt single-input tx: ' + Utils.formatAmountInBtc(new BigNumber(feePerInput * (selected.length - 1))));
          if (feeVsSingleInputFeeRatio.isGreaterThan(Defaults.UTXO_SELECTION_MAX_FEE_VS_SINGLE_UTXO_FEE_FACTOR)) {
      ...
      log.debug('Accumulated total so far: ' + Utils.formatAmountInBtc(total) + ', Net total so far: ' + Utils.formatAmountInBtc(netTotal));

      if ( total.minus(fee).isGreaterThanOrEqualTo(txpAmount)) {
        var changeAmount = total.minus(txpAmount).minus(fee);
        ...
        if (changeAmount.isGreaterThan(0) && changeAmount.isLessThanOrEqualTo(dustThreshold)) {
    ...
    if ( total.minus(fee).isLessThan(txpAmount)) {
      log.debug('Could not reach Txp total (' + Utils.formatAmountInBtc(txpAmount) + '), still missing: ' + Utils.formatAmountInBtc(txpAmount.minus(netTotal)));

      selected = [];
      if (!_.isEmpty(bigInputs)) {
        var input = _.first(bigInputs);
        log.debug('Using big input: ', Utils.formatUtxos(input));
        total = new BigNumber(input.satoshis.toFixed());
        fee = procFee(1);// Math.round(baseTxpFee + feePerInput);
        netTotal = total.minus(fee);
        selected = [input];
      }
    }
  ...
  self._getUtxosForCurrentWallet({}, function(err, utxos) {
    ...
    if (totalAmount.isLessThan(txp.getTotalAmount())) return cb(Errors.INSUFFICIENT_FUNDS);
    if (availableAmount.isLessThan(txp.getTotalAmount())) return cb(Errors.LOCKED_FUNDS);
    ...
    async.whilst(function() {
      ...
      if (!err) {
        var change = Utils.sum(txp.inputs, 'satoshis').minus(Utils.sum(txp.outputs, 'amount')).minus(txp.fee);
  ...
  self._runLocked(cb, function(cb) {
    self.getWallet({}, function(err, wallet) {
      ...
      self.storage.fetchTx(self.walletId, opts.txProposalId, function(err, txp) {
        ...
        var signingKey = self._getSigningKey(raw, opts.proposalSignature, copayer.requestPubKeys, opts.coin);
        if (!signingKey) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    async.map([wallet.network, self.walletId], function(walletId, next) {
      ...
      var notifications = _.sortBy(_.map(_.flatten(res), function(n) {
        ...
        if('amount' in n.data && typeof(n.data.amount) == "string") {
          n.data.amount = BigNumber(n.data.amount);
        }
        return n;
      }), 'id');
...
WalletService.prototype._normalizeTxHistory = function(txs) {
  ...
  return _.map([].concat(txs), function(tx) {
    ...
    var outputs = _.map(tx.vout, function(item) {
      ...
      return {
        address: itemAddr,
        amount: (new BigNumber(item.value).times('100000000')).toFixed(0),
      }
    });
    ...
    return {
      ...
      fees: (new BigNumber(tx.fees).times('100000000')).toFixed(0),
      size: tx.size,
      time: t,
      inputs: inputs,
      outputs: outputs,
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  function decorate(wallet, txs, addresses, proposals, notes) {
    ...
    function sum(items, isMine, isChange) {
      ...
      return Utils.sum(_.filter(items, filter), 'amount');
    };
    ...
    return _.map(txs, function(tx) {
      ...
      if (tx.outputs.length || tx.inputs.length) {
        ...
        if (amountIn.isEqualTo(amountOut.plus(amountOutChange.plus(amountIn.isGreaterThan(0) ? tx.fees : 0)))) {
          amount = amountOut;
          action = 'moved';
        } else {
          amount = amountIn.minus(amountOut).minus(amountOutChange).minus(amountIn.isGreaterThan(0) ? tx.fees : 0);
          action = amount.isGreaterThan(0) ? 'sent' : 'received';
        }
