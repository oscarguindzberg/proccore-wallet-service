...
Storage.prototype._completeTxData = function(walletId, txs, cb) {
  ...
  self.fetchWallet(walletId, function(err, wallet) {
    ...
    _.each([].concat(txs), function(tx) {
      ...
      tx.inputs.forEach((e, i)=>{
        e.satoshis = BigNumber(e.satoshis);
      })
      tx.outputs.forEach((e, i)=>{
        e.amount = BigNumber(e.amount);
      })
      tx.amount = BigNumber(tx.amount);
      tx.fee = BigNumber(tx.fee);
...
Storage.prototype.storeTx = function(walletId, txp, cb) {
  txp.inputs.forEach((e, i)=>{
    e.satoshis = e.satoshis.toFixed();
  })
  txp.outputs.forEach((e, i)=>{
    e.amount = e.amount.toFixed();
  })
  txp.amount = txp.amount.toFixed();
  txp.fee = txp.fee.toFixed();
